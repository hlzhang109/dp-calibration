...
from private_transformers.privacy_utils.misc import NoisyOptimizerWrapper
from private_transformers.privacy_utils.privacy_engine import PrivacyEngine
from classification.src.compiled_args import PrivacyArguments, TrainingArguments
from classification.src.dataset import FewShotDataset
from classification.src.models import (
...
from classification.src.processors import get_mappings
from classification.src.al_trainer import Trainer
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    train_size: int = field(
        default=-1,
        metadata={"help": "subsample training sets for AL"}
    )
    n_init_labeled: int = field(
        default=10000,
        metadata={"help": "# initial labeled data in the training set"}
    )
    # NOTE for splitting the valiration set for recalibration
    valid_ratio: float = field(
        default=0.1,
        metadata={"help": "validation split ratio"}
    )
    target_acc: float = field(
        default = -1, metadata={"help": "for ablation studies"}
    )
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    # HL NOTE: For AL
    strategy: str = field(
        default="LeastConfidence",
        metadata={"help": "Where to save the prediction result"}
    )
    full_training: bool = field(
        default=False,
        metadata={"help": "Use full training set or not. True means no AL"}
    )
    n_round: int = field(
        default=10,
        metadata={"help": "# round"}
    ...
    noisy_grad: float = field(
        default=0.0,
        metadata={"help": "noisy gradient hyperparameter"}
    )
    # NOTE for wandb
    report_to: str = field(
        default="wandb",
        metadata={"help": "report to wandb"}
    )
    run_name: str = field(
        default="Unnamed wandb project",
        metadata={"help": "run name"}
    )
    num_labels: int = field(
        default=3,
        metadata={"help": "num labels"}
    )
    # NOTE for label noise exp
    corruption_prob: float = field(
        default=0,
        metadata={"help": "corruption probability"}
    )
    corruption_type: str = field(
        default=None,
        metadata={"help": "corruption type - unif, flip1, flip2, clabels, hierarchical"}
    )
    max_norm_bound: float = field(
        default=100,
        metadata={"help": "max norm bound for global clipping"}
    )
    global_clip: bool = field(
        default=False, metadata={"help": "do the global gradient clipping."}
    )
    sgld: bool = field(
        default=False, metadata={"help": "do the dp sgld for baseline comparison."}
    )
...
def main(): 
    ...
    processors_mapping, num_labels_mapping, output_modes_mapping, compute_metrics_mapping, median_mapping, bound_mapping = get_mappings(data_args.task_name)
    if privacy_args.non_private:
        training_args.run_name = f"{data_args.task_name}" #_lr{training_args.learning_rate}"
    else:
        training_args.run_name = "DP_" + f"{data_args.task_name}"
    print(f"log to wandb project: {training_args.run_name}")
    ...
    elif data_args.task_name == "semantic_fragment":
        data_args.mapping = "{'contradiction': 'no', 'entailment': 'yes', 'neutral': 'maybe'}"
    elif data_args.task_name == "rte":
        data_args.mapping = "{'1': 'yes', '0': 'no'}"  # "{'contradiction': 'no', 'entailment': 'yes', 'neutral': 'maybe'}" # 1 -- equivalent, 0 -- not equivalent.
    elif data_args.task_name == "mrpc":
        data_args.mapping = "{'not_equivalent': 'no', 'equivalent': 'yes'}"
    elif data_args.task_name == "cola":
        data_args.mapping = "{'not_acceptable': 'no', 'acceptable': 'yes'}"
    else:
    ...
    if (
        ...
        and not training_args.overwrite_output_dir #compute_metrics_mapping
    ):
    ...
    config = AutoConfig.from_pretrained(
        ...
        hidden_dropout_prob=training_args.dropout_prob,
        attention_probs_dropout_prob=training_args.dropout_prob,
    )
    ...
    if model_args.few_shot_type == "finetune":
        ...
        if training_args.sgld or training_args.global_clip or data_args.valid_ratio <= 0:
            # NOTE full training for fairness 
            ...
            test_dataset = (GlueDataset(data_args, tokenizer, mode="dev") if training_args.do_predict else None)
            print(f"No validation splits | Train: {len(train_dataset)}, Eval: {len(eval_dataset)}, Test: {len(test_dataset)}")
        else:
            train_dataset = GlueDataset(data_args, tokenizer, mode="train")
            # NOTE split for recalibration
            valid_size = int(len(train_dataset) * data_args.valid_ratio)
            train_dataset, eval_dataset = torch.utils.data.random_split(train_dataset, [len(train_dataset)-valid_size, valid_size])
            test_dataset = (GlueDataset(data_args, tokenizer, mode="dev") if training_args.do_predict else None)
    ...
    def build_compute_metrics_fn(task_name: str) -> Callable[[EvalPrediction], Dict]:
        def compute_metrics_fn(p: EvalPrediction):
            ...
            return compute_metrics_mapping[task_name](task_name, preds, label_ids, torch.tensor(logits).to("cuda"))
    ...
    # RGP -- Manually select the parameters to update.
    ...
    if training_args.noisy_grad > 0 and privacy_args.non_private:
        optimizer = NoisyOptimizerWrapper(optimizer, noise_multiplier=training_args.noisy_grad)
    
    ### Corrupt
    # if training_args.corruption_prob > 0:
    #     print(f" *** corruption init {training_args.corruption_type}: {training_args.corruption_prob}")
    #     for i, feature in tqdm(enumerate(train_dataset.features)):
    #         y = [feature.label]
    #         y = trainer.corrupter.corrupt(y)[0]
    #         train_dataset.features[i].label = y
    #         print()
    
    train_dataloader, unlabeled_dataloader = trainer.get_train_dataloader()
    if training_args.lr_decay:  # Default linear decay.
        training_setup = trainer.get_training_setup(train_dataloader)
    ...
    if privacy_args.non_private:
        ...
        # normally, noise mul is roughly estimated as epochs * sample_size / batch_size # sample_rate = batch_size / sample_size
        sample_size = len(train_dataset) if not training_args.al else training_args.n_round * data_args.n_init_labeled + (training_args.n_round+1) * training_args.n_query / 2.0 / training_args.num_train_epochs
        privacy_engine = PrivacyEngine(
            ...
            noclip=privacy_args.noclip,
            global_clip=training_args.global_clip,
            sgld=training_args.sgld,
            max_norm_bound=training_args.max_norm_bound,
    ...
    if training_args.do_train:
        ...
        # Don't reload.
        trainer.train(train_dataloader, model_path=None)
    ...
    if training_args.do_eval or training_args.do_predict:
        ...
        model = model_fn.from_pretrained(training_args.output_dir, config=config) # NOTE num_labels consistency
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, eval_task_name, eval_split in zip(eval_datasets, eval_task_names, eval_splits):
            trainer.compute_metrics = build_compute_metrics_fn(data_args.task_name) # eval_dataset.args.task_name
            output = trainer.evaluate(eval_dataset=eval_dataset)
            eval_result = output.metrics

            # --- My evaluation procedure.
            ...
            if training_args.save_logit:
                ...
                for i, batch in enumerate(eval_loader):
                    ...
                    print()
                labels_from_loader = torch.cat(labels_from_loader, dim=0)
                labels = labels_from_loader.cpu().numpy()
                logits = predictions.reshape([eval_dataset.num_sample, -1, num_logits]).mean(axis=0) # test_dataset
                # labels = np.asarray([feature.label for i, feature in enumerate(eval_dataset.dataset.features) if i in eval_dataset.indices]) # NOTE dataset.Subset is different
                np.save(os.path.join(training_args.save_logit_dir,
                                        "valid-{}-{}-{}.npy".format(eval_task_name, training_args.model_id, # test_dataset.task_name
                                                            training_args.array_id)), logits)
                np.save(os.path.join(training_args.save_logit_dir,
                                        "valid-y-{}-{}-{}.npy".format(eval_task_name, training_args.model_id, # test_dataset.task_name
                                                            training_args.array_id)), labels)
                print(f"Saved validation logits and labels of shape {logits.shape}, {labels.shape}")
                print(f"Validation accuracy: {accuracy_score(labels, logits.argmax(axis=-1))}")

        output_path = os.path.join(training_args.output_dir, "final_results.json")
        utils.jdump(results_json, output_path)
        print("saved final results to {}".format(output_path))
    ...
    if training_args.do_predict:
        ...
        for test_dataset in test_datasets:
            test_task_name = test_dataset.args.task_name
            trainer.compute_metrics = build_compute_metrics_fn(test_task_name)
            output = trainer.evaluate(eval_dataset=test_dataset)
            test_result = output.metrics

            output_test_file = os.path.join(
                training_args.output_dir, f"test_results_{test_task_name}.txt"
            )
            if trainer.is_world_process_zero():
                with open(output_test_file, "w") as writer:
                    logger.info("***** Test results {} *****".format(test_task_name))
                    for key, value in test_result.items():
                        logger.info("  %s = %s", key, value)
                        writer.write("%s = %s\n" % (key, value))
                        final_result[test_task_name + '_test_' + key] = value

                if training_args.save_logit:
                    predictions = output.predictions
                    num_logits = predictions.shape[-1]
                    logits = predictions.reshape([test_dataset.num_sample, -1, num_logits]).mean(axis=0)
                    labels = np.asarray([feature.label for feature in test_dataset.features])
                    np.save(os.path.join(training_args.save_logit_dir,
                                         "test-{}-{}-{}.npy".format(test_task_name, training_args.model_id,
                                                               training_args.array_id)), logits)
                    np.save(os.path.join(training_args.save_logit_dir,
                                        "test-y-{}-{}-{}.npy".format(test_task_name, training_args.model_id, # test_dataset.task_name
                                                            training_args.array_id)), labels)
                    print(f"Saved test logits and labels of shape {logits.shape}, {labels.shape}")
                    print(f"Test accuracy: {accuracy_score(labels, logits.argmax(axis=-1))}")
