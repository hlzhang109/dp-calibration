...
from .processors import get_mappings #processors_mapping, median_mapping
from .hanlders import Handler
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        # NOTE for ALs
        if self.mode == "train":
            self.labeled_idxs = np.zeros(self.size, dtype=bool)
            tmp_idxs = np.arange(self.size)
            np.random.shuffle(tmp_idxs)
            self.labeled_idxs[tmp_idxs[:args.n_init_labeled]] = True
            self.labeled_idxs = torch.from_numpy(self.labeled_idxs)
        ...
        # TODO: When does self.features get used?
    ...
    def get_labeled_idxs(self):
        labeled_idxs = torch.from_numpy(np.arange(self.size)[self.labeled_idxs])
        return labeled_idxs # , self.handler(self.X_train[labeled_idxs], self.Y_train[labeled_idxs])
    
    def get_unlabeled_idxs(self):
        unlabeled_idxs = torch.from_numpy(np.arange(self.size)[~self.labeled_idxs])
