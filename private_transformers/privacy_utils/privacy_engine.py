...
class PrivacyEngine(object):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        self.sgld = sgld
        if self.sgld:
            self.record_snr = False
        self.max_norm_bound = max_norm_bound
        # Internals.
        # self.steps - the total number of updates already performed by the optimizer.
        self.steps = 0  # Tracks privacy spending.
        self.alpha_t = 0.005 
        self.tau = 6.08 #0.9
        self.pre_noise_scale = 0.046
        self.decay_factor = 0.028
    ...
    def exp_decay(self):
        lrate = self.alpha_t * math.exp(-self.decay_factor*self.steps)
        return lrate
    ...
    def _step(self, loss, scale=1.):
        ...
        for name, param in self.named_params:
            ...
            if self.noise_multiplier > 0 and self.max_grad_norm > 0:
                if self.sgld:
                    # NOTE aneal the alpha learning rate
                    # self.alpha_t = self.optimizer.param_groups[0]['lr']
                    self.noise_multiplier = math.sqrt(2*self.alpha_t*self.tau)
                    self.alpha_t = self.exp_decay()
                noise = torch.normal(
                    mean=0,
    ...
    def _accumulate_summed_grad(self, loss, scale=1.):
        ...
        # NOTE don't scale grads - self.max_grad_norm * 
        coef_sample = torch.ones_like(norm_sample) if self.noclip \
            else torch.clamp_max(self.max_grad_norm * scale / (norm_sample + self.numerical_stability_constant), 1.)

        if self.global_clip and self.steps * self.gradient_accumulation_steps > 3*self.sample_size: # max_grad_norm = R, max_norm_bound = Z
            # mask = (norm_sample > self.max_norm_bound).float()
            # coef_sample = self.max_grad_norm / self.max_norm_bound * torch.ones_like(norm_sample) 
            # coef_sample = mask * coef_sample
            R = self.max_grad_norm
            Z = self.max_norm_bound
            coef_sample=torch.where(coef_sample > R/Z, torch.ones_like(coef_sample)*R/Z, torch.zeros_like(coef_sample))
        ...
        for name, param in self.named_params:
            ...
            if self.sgld:
                param.grad_sample += torch.normal(0, self.pre_noise_scale, size=param.grad_sample.shape, device=current_device)
            param.summed_grad += torch.einsum("i,i...->...", coef_sample.to(current_device), param.grad_sample) # [10], [10,3] -> [3]
